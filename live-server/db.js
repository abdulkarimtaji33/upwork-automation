...
function upsertJob(job, analysis, emailSent = false) {
  ...
  const record = {
    ...
    total_spent: job.totalSpent != null ? String(job.totalSpent) : '',
    total_hires: job.totalHires != null ? String(job.totalHires) : '',
    hire_rate: job.hireRate != null ? String(job.hireRate) : '',
    member_since: job.memberSince ?? '',
    client_info: job.clientInfo ?? '',
    full_description: job.fullDescription ?? '',
    score: analysis.relevanceScore != null ? Number(analysis.relevanceScore) : null,
